...
def main() -> None:
    ...
    append_event(conn, envelope("move", "world", "alice",
                                 {"agent_id": "alice", "name": "Alice", "x": 1, "y": 1}))
    append_event(conn, envelope("move", "world", "bob",
                                 {"agent_id": "bob", "name": "Bob", "x": 5, "y": 5}))
    append_event(conn, envelope("move", "alice", "world",
                                 {"agent_id": "alice", "x": 2, "y": 3}))
    append_event(conn, envelope("leave", "world", "bob", {"agent_id": "bob"}))
    ...
    print("W3 PASS — move/leave mutate state correctly; pump is idempotent")
