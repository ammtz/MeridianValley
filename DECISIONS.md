...
| 11 | 2026-09-25 | **Nothing public carries the Product Owner's name without their specific approval.** Builders never sign as the Product Owner: no byline, authorship, copyright line, or attribution in their name on anything they have not personally read and edited. Drafts use the placeholder `[author · pending Product Owner approval]` until approved. Applies to every repo, every platform, every Builder. | The name is the Product Owner's signature; only they sign |
| 12 | 2026-09-18 | **Proposed, awaiting Product Owner sign-off.** The repository is MIT licensed. Recorded here because the fact was load-bearing elsewhere and undocumented: `LICENSE` was added 2026-08-14 (`9e12bf7`), the copyright line was made hypothetical 2026-08-15 (`ec1df28`), then set to the author 2026-08-22 (`c798a82`). Verified against git history, not asserted. | The licence status of this repo is cited by the disclosure set; an undocumented fact there has already caused one false claim |
| 13 | 2026-09-18 | **Proposed, awaiting Product Owner sign-off.** The door (`envelopes.validate`) checks verb, mood and world-verb payload before an envelope may be appended. An event that reaches the log and cannot be applied is written to a `quarantine` state table with its reason, and the Worker's cursor advances past it. | Invariant #3 promised rejection at the door but only the verb was checked; the cursor advanced only on success, so one malformed envelope stopped the world permanently and silently. Quarantine is derived state, so replay stays byte-identical |
